...
pwd
cp -fr /apps/hosting/configs/deploy/smart.tar.gz /backup/build/veritaz/uat/uat_build_`date -%d%m%y-%H`.tar.gz
#tar -czvf /backup/build/veritaz/uat/uat_build_`date -%d%m%y-%H`.tar.gz /apps/hosting/configs/deploy/
rm -fr /apps/hosting/configs/deploy/*
tar -czf /backup/code/veritaz/uat/uat_`date -%d%m%y-%H`.tar.gz /apps/hosting/configs/smart
...
git log -10 --stat | mail -s "GIT Log SMART UAT" qabuild@raksanconsulting.com,smartteam@raksanconsulting.com,suresh.garimella@raksan.in
}
...
meteor bundle /apps/hosting/configs/deploy/smart.tar.gz
}

patch() {
cd /apps/hosting/configs/deploy/
pwd
tar -xzf smart.tar.gz
...
cp -fr /apps/hosting/configs/smart/settings-uat.json /apps/hosting/configs/deploy/bundle/
cp -fr /apps/hosting/configs/smart/settings-uat.json /apps/hosting/configs/deploy/
