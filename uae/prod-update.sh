...
pwd
cp -fr /apps/hosting/configs/deploy/smart.tar.gz /backup/build/uae/uaeprod/uaeprod_build_`date -%d%m%y-%H`.tar.gz
#tar -czvf /backup/build/uae/uaeprod/uaeprod_build_`date -%d%m%y-%H`.tar.gz /apps/hosting/configs/deploy/
rm -fr /apps/hosting/configs/deploy/*
tar -czf /backup/code/uae/prod/prod_`date -%d%m%y-%H`.tar.gz /apps/hosting/configs/smart
...
git log -10 --stat | mail -s "GIT Log Fieldrepo UAE PROD" qabuild@raksanconsulting.com,smartteam@raksanconsulting.com,suresh.garimella@raksan.in
}
